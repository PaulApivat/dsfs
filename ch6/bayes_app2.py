...
for i in range(pop):
    ...
    if has_covid:
        tests_positive = True
        if random() < 0.1: # false negative at 10%
            tests_positive = False
    else:
        tests_positive = False
        if random() < 0.01: # false positive at 1%
            tests_positive = True
...
print('probability that a test-positive person with symptoms actually has covid: %.2f' % (100.0 * counts[(True, True)] / n_positive),)
